...
namespace wasm3 {
    ...
    public:
        ...
        module parse_module(const std::shared_ptr<std::vector<uint8_t>> data);
    ...
    protected:
        ...
        module(const std::shared_ptr<M3Environment> &env, std::istream &in_wasm) {
            m_moduleRawData = std::make_shared<std::vector<uint8_t>>();
        ...
        module(const std::shared_ptr<M3Environment> &env, const uint8_t *data, size_t size) : m_env(env) {
            m_moduleRawData = std::make_shared<std::vector<uint8_t>>(data, data + size);
            parse(env.get(), m_moduleRawData->data(), m_moduleRawData->size());
        }

        module(const std::shared_ptr<M3Environment> &env, const std::shared_ptr<std::vector<uint8_t>> &module_data) 
            : m_env(env),
            m_moduleRawData(module_data) {
            parse(env.get(), m_moduleRawData->data(), m_moduleRawData->size());
        }

        void parse(IM3Environment env, const uint8_t *data, size_t size) {
            tagged_module *tagged_mod = new tagged_module({nullptr, false});
            M3Result err = m3_ParseModule(env, &(tagged_mod->module), data, size);
            detail::check_error(err);
            m_taggedModule.reset(tagged_mod, [this](tagged_module* tm) {
                if (!tm->is_loaded) {
                    m3_FreeModule(tm->module);
                }
        ...
        struct tagged_module {
            IM3Module module; 
            bool is_loaded;
        };

        std::shared_ptr<M3Environment> m_env;
        std::shared_ptr<tagged_module> m_taggedModule = std::make_shared<tagged_module>(tagged_module {nullptr, false});
        std::shared_ptr<std::vector<uint8_t>> m_moduleRawData;
    };
    ...
    inline module environment::parse_module(const std::shared_ptr<std::vector<uint8_t>> data) {
        return module(m_env, data);
    }
    ...
    void module::link(const char *module, const char *function_name, Func *function) {
        M3Result ret = detail::m3_wrapper<Func>::link(m_taggedModule->module, module, function_name, function);
